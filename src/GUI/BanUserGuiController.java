 ...
 * @author Slimen OUNI
 */
public class BanUserGuiController implements Initializable {
 ...
 private Parent root;   
  ...
  private final ObservableList<User> dataList = FXCollections.observableArrayList();
    ...
    public void searchAndFilter(){
     ...
     @FXML
    private void block(MouseEvent event) {
        
        int userId = rowClicked(event);
        
        UserService us = new UserService();
       

                         Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Vous allez désactiver l'acces a ce utilisateur?");
        alert.setHeaderText("Vous allez blocker l'utilisateur");
        Optional<ButtonType> result = alert.showAndWait();
        if (result.get() == ButtonType.OK) {
            us.updatesetAccountStatus(userId, false);
        } else {
            alert.close();
        }
  ...
  stage.show();
    ...
    @FXML
    private int rowClicked(MouseEvent event) {
        User clickedUser = userList.getSelectionModel().getSelectedItem();
        return clickedUser.getId();
    }
